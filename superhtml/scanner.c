...
static inline void advance_superhtml(TSLexer *lexer) { lexer->advance_superhtml(lexer, false); }

static inline void skip_superhtml(TSLexer *lexer) { lexer->advance_superhtml(lexer, true); }

static unsigned serialize_superhtml(Scanner *scanner, char *buffer) {
...
static void deserialize_superhtml(Scanner *scanner, const char *buffer, unsigned length) {
...
static bool scan_superhtml(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
    if (valid_symbols[RAW_TEXT] && !valid_symbols[START_TAG_NAME] && !valid_symbols[END_TAG_NAME]) {
        return scan_raw_text(scanner, lexer);
    }

    while (iswspace(lexer->lookahead)) {
        skip_superhtml(lexer);
    }
...
void *tree_sitter_superhtml_external_scanner_create() {
    Scanner *scanner = (Scanner *)ts_calloc(1, sizeof(Scanner));
    return scanner;
}

bool tree_sitter_superhtml_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    Scanner *scanner = (Scanner *)payload;
    return scan_superhtml(scanner, lexer, valid_symbols);
}

unsigned tree_sitter_superhtml_external_scanner_serialize(void *payload, char *buffer) {
    Scanner *scanner = (Scanner *)payload;
    return serialize_superhtml(scanner, buffer);
}

void tree_sitter_superhtml_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
    Scanner *scanner = (Scanner *)payload;
    deserialize_superhtml(scanner, buffer, length);
}

void tree_sitter_superhtml_external_scanner_destroy(void *payload) {
    Scanner *scanner = (Scanner *)payload;
    for (unsigned i = 0; i < scanner->tags.size; i++) {
        tag_free(&scanner->tags.contents[i]);
    }
