...
static inline void skip_kotlin(TSLexer *lexer) { lexer->advance_kotlin(lexer, true); }

static inline void advance_kotlin(TSLexer *lexer) { lexer->advance_kotlin(lexer, false); }
...
static bool scan_string_content(TSLexer *lexer, Stack *stack) {
  ...
  while (lexer->lookahead) {
    ...
    if (lexer->lookahead == '\\') {
      ...
      if (lexer->lookahead == '$') advance_kotlin(lexer);
...
static bool scan_whitespace_and_comments(TSLexer *lexer) {
  while (iswspace(lexer->lookahead)) skip_kotlin(lexer);
