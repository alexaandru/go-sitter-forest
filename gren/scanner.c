...
static inline void advance_gren(TSLexer *lexer) { lexer->advance_gren(lexer, false); }

static inline void skip_gren(TSLexer *lexer) { lexer->advance_gren(lexer, true); }
...
static bool is_gren_space(TSLexer *lexer) {
...
static int checkForIn(TSLexer *lexer, const bool *valid_symbols) {
  ...
  if (valid_symbols[VIRTUAL_END_SECTION] && lexer->lookahead == 'i') {
    ...
    if (lexer->lookahead == 'n') {
      skip_gren(lexer);
      if (is_gren_space(lexer) || lexer->eof(lexer)) {
        return 2; // Success
      }
      return 1; // Partial
    }
...
static bool scan_gren(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
 ...
 */
void *tree_sitter_gren_external_scanner_create() {
 ...
 */
bool tree_sitter_gren_external_scanner_scan(void *payload, TSLexer *lexer,
                                            const bool *valid_symbols) {
  Scanner *scanner = (Scanner *)payload;
  return scan_gren(scanner, lexer, valid_symbols);
}
 ...
 */
unsigned tree_sitter_gren_external_scanner_serialize(void *payload,
                                                     char *buffer) {
  Scanner *scanner = (Scanner *)payload;
  size_t size = 0;
 ...
 * parsing).
 */
void tree_sitter_gren_external_scanner_deserialize(void *payload,
                                                   const char *buffer,
                                                   unsigned length) {
  Scanner *scanner = (Scanner *)payload;
  VEC_CLEAR(scanner->runback);
 ...
 * Destroy the state.
 */
void tree_sitter_gren_external_scanner_destroy(void *payload) {
  Scanner *scanner = (Scanner *)payload;
