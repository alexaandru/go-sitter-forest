package pem_test

import (
	"context"
	"testing"

	"github.com/alexaandru/go-tree-sitter-parsers/pem"
	sitter "github.com/smacker/go-tree-sitter"
)

const (
	code = `
# Private key of the SSL certificate
-----BEGIN PRIVATE KEY-----

z3P668YfhUbKdRF6S42Cg6zn
-----END PRIVATE KEY-----

# Public key of the SSL certificate (the server certificate)
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	expected = "(pem (comment) (content (header (dashes) (label) (dashes)) (data) (footer (dashes) (label) (dashes))) (comment) (content (header (dashes) (label) (dashes)) (data) (footer (dashes) (label) (dashes))))"
)

func TestGrammar(t *testing.T) {
	n, err := sitter.ParseCtx(context.Background(), []byte(code), pem.GetLanguage())
	if err != nil {
		t.Fatalf("Expected no error got %v", err)
	}

	if act := n.String(); act != expected {
		t.Fatalf("Expected %q got %q", expected, act)
	}
}
