...
static inline void consume(TSLexer *lexer) { lexer->advance_ldg(lexer, false); }

static inline void skip_ldg(TSLexer *lexer) { lexer->advance_ldg(lexer, true); }
...
void *tree_sitter_ldg_external_scanner_create() {
  Scanner *scanner = calloc(1, sizeof(Scanner));
  return scanner;
}

void tree_sitter_ldg_external_scanner_destroy(void *payload) {
  Scanner *scanner = (Scanner *)payload;
  free(scanner);
}

unsigned tree_sitter_ldg_external_scanner_serialize(void *payload, char *buffer) {
  Scanner *scanner = (Scanner *)payload;
...
void tree_sitter_ldg_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
  Scanner *scanner = (Scanner *)payload;
  if (length == 0) return;
...
bool tree_sitter_ldg_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
  Scanner *scanner = (Scanner *)payload;
