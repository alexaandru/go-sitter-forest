...
static inline void advance_sage(TSLexer *lexer) { lexer->advance_sage(lexer, false); }

static inline void skip_sage(TSLexer *lexer) { lexer->advance_sage(lexer, true); }
            ...
            if (lexer->lookahead == '\\') {
                ...
                if (is_bytes(delimiter)) {
                    ...
                    if (lexer->lookahead == 'N' || lexer->lookahead == 'u' || lexer->lookahead == 'U') {
                        ...
                        // https://docs.sage.org/3/reference/lexical_analysis.html#string-and-bytes-literals
                        advance_sage(lexer);
                    } else {
                        lexer->result_symbol = STRING_CONTENT;
                        return has_content;
