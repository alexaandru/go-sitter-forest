...
static inline void advance_test(TSLexer *lexer) { lexer->advance_test(lexer, false); }

static inline bool scan_test(TSLexer *lexer, Scanner *scanner, char chr, int symbol) {
    uint32_t length;
    ...
    if (lexer->lookahead == '\r') advance_test(lexer);
    if (lexer->lookahead == '\n') advance_test(lexer);
...
bool tree_sitter_test_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    ...
    if (valid_symbols[EQUALS_BEGIN])
        return scan_test(lexer, scanner, '=', EQUALS_BEGIN);

    if (valid_symbols[EQUALS_END])
        return scan_test(lexer, scanner, '=', EQUALS_END);

    if (valid_symbols[DASHES])
        return scan_test(lexer, scanner, '-', DASHES);
