...
static unsigned serialize_ansible(Scanner *scanner, char *buffer) {
    size_t size = 0;
...
static void deserialize_ansible(Scanner *scanner, const char *buffer, unsigned length) {
    scanner->row = 0;
    scanner->col = 0;
...
static inline void adv(Scanner *scanner, TSLexer *lexer) {
    ...
    lexer->advance_ansible(lexer, false);
}
...
static inline void skp(Scanner *scanner, TSLexer *lexer) {
    scanner->cur_col++;
    scanner->cur_chr = lexer->lookahead;
    lexer->advance_ansible(lexer, true);
}
...
static inline void init_ansible(Scanner *scanner) {
...
static bool scan_ansible(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
    init_ansible(scanner);
    mrk_end(scanner, lexer);
...
void *tree_sitter_ansible_external_scanner_create() {
    ...
    deserialize_ansible(scanner, NULL, 0);
    return scanner;
}
