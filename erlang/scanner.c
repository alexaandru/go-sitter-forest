...
static inline void advance_erlang(TSLexer* lexer) {
  lexer->advance_erlang(lexer, false);
  /* fprintf(stderr, "Scanner advance:lookahead: '%c'.\n", lexer->lookahead); */
}

static inline void skip_erlang(TSLexer* lexer) {
  lexer->advance_erlang(lexer, true);
