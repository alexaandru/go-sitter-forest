...
`
	expected = "(source_file (version_command version: (version_major_minor)) base_target: (block (from_command (image_spec name: (image_name) tag: (image_tag))) (workdir_command (string (unquoted_string)))) (target name: (identifier) (block (copy_command src: (string (unquoted_string)) dest: (string (unquoted_string))) (run_command command: (shell_fragment)) (save_artifact_command src: (string (unquoted_string)) local_dest: (string (unquoted_string))))) (target name: (identifier) (block (copy_command src: (target_artifact (target_ref name: (identifier)) (unquoted_string)) dest: (string (unquoted_string))) (entrypoint_command (string_array (double_quoted_string))) (save_image_command images: (images (image_spec name: (image_name) tag: (image_tag)))))))"
)

func TestGrammar(t *testing.T) {
	n, err := sitter.ParseCtx(context.Background(), []byte(code), earthfile.GetLanguage())
	if err != nil {
