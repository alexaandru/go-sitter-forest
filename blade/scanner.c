...
static inline void advance_blade(TSLexer *lexer) { lexer->advance_blade(lexer, false); }

static inline void skip_blade(TSLexer *lexer) { lexer->advance_blade(lexer, true); }

static unsigned serialize_blade(Scanner *scanner, char *buffer) {
    uint16_t tag_count =
...
static void deserialize_blade(Scanner *scanner, const char *buffer, unsigned length) {
        ...
        if (tag_count > 0) {
            ...
            for (iter = 0; iter < serialized_tag_count; iter++) {
                ...
                if (tag.type == CUSTOM) {
                    ...
                    memcpy(tag.custom_tag_name.contents, &buffer[size],
                           name_length);
                    size += name_length;
                }
...
static bool scan_blade(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
...
bool tree_sitter_blade_external_scanner_scan(void *payload, TSLexer *lexer,
    ...
    return scan_blade(scanner, lexer, valid_symbols);
}

unsigned tree_sitter_blade_external_scanner_serialize(void *payload,
                                                      char *buffer) {
    Scanner *scanner = (Scanner *)payload;
    return serialize_blade(scanner, buffer);
}

void tree_sitter_blade_external_scanner_deserialize(void *payload,
                                                    const char *buffer,
                                                    unsigned length) {
    Scanner *scanner = (Scanner *)payload;
    deserialize_blade(scanner, buffer, length);
}
