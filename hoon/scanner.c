...
bool tree_sitter_hoon_external_scanner_scan(void *payload, TSLexer *lexer,
    ...
    if (valid_symbols[STRING_CONTENT] && scanner->delimiters.len > 0) {
        ...
        while (lexer->lookahead) {
            if ((lexer->lookahead == '{') && delimiter.flags & DoubleQuote) {
...
unsigned tree_sitter_hoon_external_scanner_serialize(void *payload,
                                                       char *buffer) {
    Scanner *scanner = (Scanner *)payload;
...
void tree_sitter_hoon_external_scanner_deserialize(void *payload,
                                                     const char *buffer,
                                                     unsigned length) {
    Scanner *scanner = (Scanner *)payload;
...
void *tree_sitter_hoon_external_scanner_create() {
    ...
    tree_sitter_hoon_external_scanner_deserialize(scanner, NULL, 0);
    return scanner;
}

void tree_sitter_hoon_external_scanner_destroy(void *payload) {
    Scanner *scanner = (Scanner *)payload;
