...
static inline void advance_ocaml(TSLexer *lexer) { lexer->advance_ocaml(lexer, false); }

static inline void skip_ocaml(TSLexer *lexer) { lexer->advance_ocaml(lexer, true); }
...
static int32_t scan_character(TSLexer *lexer) {
  ...
  switch (lexer->lookahead) {
    ...
    case '\r':
      advance_ocaml(lexer);
      while (lexer->lookahead == '\r') advance_ocaml(lexer);
      if (lexer->lookahead != '\n') return 0;
      advance_ocaml(lexer);
      break;
    case '\0':
...
static inline bool scan_identifier(TSLexer *lexer) {
  if (is_identstart(lexer->lookahead)) {
    advance_ocaml(lexer);
    while (is_identchar(lexer->lookahead)) advance_ocaml(lexer);
    return true;
  }
  return false;
}
  ...
  for (;;) {
    switch (last ? last : lexer->lookahead) {
      ...
      case '{':
        ...
        if (lexer->lookahead == '%') {
          advance_ocaml(lexer);
          if (lexer->lookahead == '%') advance_ocaml(lexer);
          if (scan_extattrident(lexer)) {
            while (iswspace(lexer->lookahead)) advance_ocaml(lexer);
          } else {
            break;
          }
        }
        if (scan_quoted_string(scanner, lexer)) advance_ocaml(lexer);
        break;
      case '\0':
...
static bool scan_ocaml(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
  ...
  if (!scanner->in_string && valid_symbols[LINE_NUMBER_DIRECTIVE] &&
    ...
    while (iswdigit(lexer->lookahead)) advance_ocaml(lexer);
