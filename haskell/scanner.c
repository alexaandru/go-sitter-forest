 ...
 */
static void advance_haskell(Env *env) {
  if (not_eof(env)) {
    array_push(&env->state->lookahead, PEEK);
    env->lexer->advance_haskell(env->lexer, false);
  }
}
...
static void skip_debug(Env *env) {
  append_parse_buffer(env);
  env->lexer->advance_haskell(env->lexer, true);
}
...
static bool is_newline_haskell(int32_t c) {
  switch (c) {
    NEWLINE_CASES:
      return true;
    default:
...
static ParseLine *ensure_parse_buffer(Env *env) {
  ...
  if (is_newline_haskell(PEEK)) push_parse_buffer_line(env);
  return array_back(buffer);
}

static void append_parse_buffer(Env *env) {
  ParseLine *current_line = ensure_parse_buffer(env);
  if (is_newline_haskell(PEEK)) {
...
static void fill_parse_buffer(Env *env) {
  ...
  while (!(is_newline_haskell(PEEK) || is_eof(env))) S_ADVANCE;
}
...
static void take_line(Env *env) {
  while (not_eof(env) && !is_newline_haskell(PEEK)) S_ADVANCE;
}
...
static void take_line_escaped_newline(Env *env) {
  for (;;) {
    while (not_eof(env) && !is_newline_haskell(PEEK) && PEEK != '\\') S_ADVANCE;
    if (PEEK == '\\') {
      S_ADVANCE;
      if (is_space_or_tab(PEEK)) {
        while (is_space_or_tab(PEEK)) S_ADVANCE;
        if (is_newline_haskell(PEEK)) S_ADVANCE;
      }
      else S_ADVANCE;
    }
    else return;
...
static bool skip_newlines(Env *env) {
  if (!is_newline_haskell(PEEK)) return false;
  S_SKIP;
  while(is_newline_haskell(PEEK)) S_SKIP;
  return true;
}
...
static bool cpp_directive_other(Env *env, uint32_t start) {
  return
    ...
    is_newline_haskell(peek(env, start))
    ||
    // shebang for scripts
...
static Symbol scan_main(Env *env) {
  ...
  if (is_newline_haskell(PEEK)) return newline_start(env);
...
static bool scan_haskell(Env *env) {
