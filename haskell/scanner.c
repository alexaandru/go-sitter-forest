...
State state_new_haskell(TSLexer *l, const bool * restrict vs, indent_vec *is) {
  return (State) {
    .lexer = l,
    .symbols = vs,
    .indents = is,
...
static bool is_newline_haskell(uint32_t c) {
  switch (c) {
    NEWLINE_CASES:
      return true;
    default:
...
static void cpp_consume(State *state) {
  for (;;) {
    while (PEEK != 0 && !is_newline_haskell(PEEK) && PEEK != '\\') S_ADVANCE;
    if (PEEK == '\\') {
      S_ADVANCE;
      S_ADVANCE;
      continue;
