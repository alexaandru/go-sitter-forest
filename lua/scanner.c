...
static inline void consume(TSLexer *lexer) { lexer->advance_lua(lexer, false); }

static inline void skip_lua(TSLexer *lexer) { lexer->advance_lua(lexer, true); }
...
unsigned tree_sitter_lua_external_scanner_serialize(void *payload, char *buffer) {
  Scanner *scanner = (Scanner *)payload;
  buffer[0] = scanner->ending_char_lua;
...
void tree_sitter_lua_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
  Scanner *scanner = (Scanner *)payload;
  if (length == 0) return;
  scanner->ending_char_lua = buffer[0];
  if (length == 1) return;
...
static bool scan_comment_content(Scanner *scanner, TSLexer *lexer) {
  if (scanner->ending_char_lua == 0) { // block comment
  ...
  while (lexer->lookahead != 0) {
    if (lexer->lookahead == scanner->ending_char_lua) {
      reset_state(scanner);
      lexer->result_symbol = BLOCK_COMMENT_CONTENT;
      return true;
    }
...
bool tree_sitter_lua_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
  ...
  if (valid_symbols[BLOCK_COMMENT_END] && scanner->ending_char_lua == 0 && scan_block_end(scanner, lexer)) {
    reset_state(scanner);
