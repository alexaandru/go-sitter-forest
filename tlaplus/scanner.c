...
#define END_LEX_STATE()   \
   ...
   */
  static void advance_tlaplus(TSLexer* const lexer) {
    lexer->advance_tlaplus(lexer, false);
  }
  ...
  struct Scanner {
    ...
    static bool is_junct_token_higher_level_op_parameter(TSLexer* const lexer) {
      while (iswspace(lexer->lookahead) && has_next(lexer)) {
        lexer->advance_tlaplus(lexer, true);
      }
  ...
  struct NestedScanner {
    ...
    static bool nested_scan(
      ...
      if (valid_symbols[ERROR_SENTINEL]) {
        ...
        return scan_tlaplus(&this->current_context, lexer, valid_symbols);
      }
    }
