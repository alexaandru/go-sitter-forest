...
static Tag make_tag(TagType type, const char *name) {
    ...
    if (type == CUSTOM) {
        ...
        tag.custom_tag_name.data = (char *)calloc(1, sizeof(char) * (tag.custom_tag_name.len + 1));
...
static inline Tag for_name(const char *name) { return make_tag(get_tag_from_string(name), name); }

static inline bool tagcmp(const Tag *_tag1, const Tag *_tag2) {
    return _tag1->type == _tag2->type &&
           (_tag1->type == CUSTOM ? strcmp(_tag1->custom_tag_name.data, _tag2->custom_tag_name.data) == 0 : true);
}
