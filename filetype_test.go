...
func TestDetectLanguage(t *testing.T) { //nolint:funlen,tparallel // no, subtests MUST NOT call t.Parallel here!
	...
	testCases := []struct{ s, exp string }{
		...
		{"Neomuttrc", "muttrc"},
		{"Neomuttrc-bar", "muttrc"},
		{"Neomuttrc-foo", "muttrc"},
		{"PKGBUILD", "bash"},
		{"Pipfile", "toml"},
		{"Pipfile.lock", "json"},
		{"Puppetfile", "ruby"},
		...
		{"foo/.muttngrc", "muttrc"},
		{"foo/.muttngrcbar", "muttrc"},
		{"foo/.muttrc", "muttrc"},
		{"foo/.muttrcbar", "muttrc"},
		{"foo/.neomuttrc", "muttrc"},
		{"foo/.neomuttrcbar", "muttrc"},
...
func TestExtractFromShebang(t *testing.T) {
	...
	for _, tc := range append(ft.shebangs(), SupportedLanguages()...) {
		if tc == "json5" {
			// This one fails detection but also is not an interpreter, so who cares?
			continue
		}
