...
static void advance_rescript(TSLexer *lexer) { lexer->advance_rescript(lexer, false); }
static void skip_rescript(TSLexer *lexer) { lexer->advance_rescript(lexer, true); }
...
static void scan_whitespace(TSLexer *lexer, bool skip) {
  while (iswspace(lexer->lookahead) && !lexer->eof(lexer)) {
    lexer->advance_rescript(lexer, skip);
  }
}

static void scan_multiline_comment(TSLexer *lexer) {
  int level = 1;
  advance_rescript(lexer);
...
bool tree_sitter_rescript_external_scanner_scan(
  ...
  if (valid_symbols[NEWLINE] && lexer->lookahead == '\n') {
    ...
    lexer->result_symbol = NEWLINE;
    lexer->advance_rescript(lexer, true);
    lexer->mark_end(lexer);
  ...
  if (valid_symbols[QUOTE] && lexer->lookahead == '"') {
    ...
    lexer->advance_rescript(lexer, false);
    lexer->mark_end(lexer);
    return true;
  }
  ...
  lexer->advance_rescript(lexer, iswspace(lexer->lookahead));
  return false;
}
