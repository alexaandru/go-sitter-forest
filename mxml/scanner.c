...
static inline void advance_mxml(TSLexer *lexer) { lexer->advance_mxml(lexer, false); }

static inline void skip_mxml(TSLexer *lexer) { lexer->advance_mxml(lexer, true); }

static unsigned serialize_mxml(Scanner *scanner, char *buffer) {
...
static void deserialize_mxml(Scanner *scanner, const char *buffer, unsigned length) {
...
static bool scan_mxml(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
...
void *tree_sitter_mxml_external_scanner_create() {
    Scanner *scanner = (Scanner *)calloc(1, sizeof(Scanner));
    return scanner;
}

bool tree_sitter_mxml_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    Scanner *scanner = (Scanner *)payload;
    return scan_mxml(scanner, lexer, valid_symbols);
}

unsigned tree_sitter_mxml_external_scanner_serialize(void *payload, char *buffer) {
    Scanner *scanner = (Scanner *)payload;
    return serialize_mxml(scanner, buffer);
}

void tree_sitter_mxml_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
    Scanner *scanner = (Scanner *)payload;
    deserialize_mxml(scanner, buffer, length);
}

void tree_sitter_mxml_external_scanner_destroy(void *payload) {
    Scanner *scanner = (Scanner *)payload;
    for (unsigned i = 0; i < scanner->tags.size; i++) {
        tag_free(&scanner->tags.contents[i]);
    }
